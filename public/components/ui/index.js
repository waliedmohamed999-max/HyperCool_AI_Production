...
export function drawer(title,node,{restore=false}={}){
 ...
 const head=document.createElement('div');head.className='dialog-head';head.innerHTML=`<div><span class="eyebrow">HYPERCOOL</span><h2 id="${id}">${escape(title)}</h2></div>`;
 const close=button('إغلاق',{variant:'ghost',iconName:'close'});close.onclick=()=>dialog.close();head.append(close);
...
export function dataTable(table,{pageSize=10}={}){
 ...
 const toolbar=document.createElement('div');toolbar.className='table-toolbar';const search=document.createElement('input');search.type='search';search.placeholder='بحث في الجدول…';search.setAttribute('aria-label','بحث في الجدول');toolbar.append(search);
 const scroll=document.createElement('div');scroll.className='table-scroll';scroll.tabIndex=0;scroll.setAttribute('role','region');scroll.setAttribute('aria-label','جدول بيانات قابل للتمرير');scroll.append(table);
 const footer=document.createElement('div');footer.className='table-footer';const prev=button('السابق'),next=button('التالي'),count=document.createElement('span');count.setAttribute('aria-live','polite');footer.append(count,prev,next);wrapper.append(toolbar,scroll,footer);
 ...
 for(const index of filterColumns){const select=document.createElement('select');select.setAttribute('aria-label','تصفية حسب '+table.tHead.rows[0].cells[index].textContent);select.innerHTML=`<option value="">كل ${escape(table.tHead.rows[0].cells[index].textContent)}</option>`+[...new Set(rows.map(row=>row.cells[index].textContent.trim()))].map(value=>`<option>${escape(value)}</option>`).join('');select.onchange=()=>{page=0;draw();};toolbar.append(select);filters.push({index,select});}
 function draw(){const filtered=rows.filter(row=>row.textContent.toLocaleLowerCase().includes(search.value.trim().toLocaleLowerCase())&&filters.every(({index,select})=>!select.value||row.cells[index].textContent.trim()===select.value));if(sort>=0)filtered.sort((a,b)=>a.cells[sort].textContent.localeCompare(b.cells[sort].textContent,'ar',{numeric:true})*direction);page=Math.min(page,Math.max(0,Math.ceil(filtered.length/pageSize)-1));rows.forEach(row=>row.hidden=true);filtered.forEach((row,index)=>{table.tBodies[0].append(row);row.hidden=index<page*pageSize||index>=(page+1)*pageSize;});count.textContent=filtered.length?`${page*pageSize+1}–${Math.min((page+1)*pageSize,filtered.length)} من ${filtered.length}`:'لا توجد نتائج مطابقة';prev.disabled=page===0;next.disabled=(page+1)*pageSize>=filtered.length;}
...
export function tabs(container,entries){const bar=document.createElement('div');bar.className='ui-tabs';bar.setAttribute('role','tablist');bar.setAttribute('aria-label','أقسام الصفحة');container.prepend(bar);const buttons=[];
...
export function enhance(root=document){
 ...
 root.querySelectorAll('input,select,textarea').forEach(el=>{if(!el.labels?.length&&!el.hasAttribute('aria-label'))el.setAttribute('aria-label',el.placeholder||el.name||'حقل');if(['url','email','tel','date','datetime-local'].includes(el.type))el.dir='ltr';});
