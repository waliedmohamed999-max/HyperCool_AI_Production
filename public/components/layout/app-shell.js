...
const ROUTE_ICONS={overview:'grid','command-center':'agent',workflows:'clock',marketing:'chart',whatsapp:'plug',crm:'users',planning:'calendar',reports:'chart',content:'file',agents:'agent',knowledge:'book',integrations:'plug','control-center':'plug',onboarding:'check',audit:'clock',users:'users',account:'info',platform:'grid'};
...
export function installShell(){
 ...
 for(const link of nav.querySelectorAll('a')){const id=link.hash.slice(1);link.innerHTML=`<span class="nav-icon"></span><span></span>`;link.onclick=()=>{builderNav=link.dataset.routeKey==='integrationBuilder';updateNavCurrent(link.hash.slice(1));closeMobile();};}
...
// "Platform" and "Integration Builder" share the #platform route, so both matched the hash and
// rendered as active together. Only the entry the admin actually clicked (or Platform by default) is current.
let builderNav=false;
function updateNavCurrent(route){document.querySelectorAll('nav a').forEach(a=>{const isBuilder=a.dataset.routeKey==='integrationBuilder';const current=a.hash==='#'+route&&(route!=='platform'||isBuilder===builderNav);if(current)a.setAttribute('aria-current','page');else a.removeAttribute('aria-current');});}
export function shellPage(route){if(route!=='platform')builderNav=false;document.querySelectorAll('[data-mobile-route]').forEach(b=>{const active=b.dataset.mobileRoute===route;b.classList.toggle('active',active);if(active)b.setAttribute('aria-current','page');else b.removeAttribute('aria-current');});document.querySelector('.mobile-more')?.classList.toggle('active',!['overview','crm','content','agents'].includes(route));document.querySelector('#breadcrumb-page').textContent=routeTitle(route)||'';document.title=`${routeTitle(route)} | ${t('common.appName')}`;updateNavCurrent(route);const idx=railGroupIndex(route);document.querySelectorAll('.rail-icon').forEach((b,i)=>b.classList.toggle('active',i===idx));}
