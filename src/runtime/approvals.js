...
export function installApprovals(db) {
 ...
 if(legacy) {
  ...
  db.exec('CREATE TABLE IF NOT EXISTS agent_approvals (id TEXT PRIMARY KEY, tenant_id TEXT, run_id TEXT, agent_id TEXT NOT NULL, action_type TEXT NOT NULL, proposed_output TEXT NOT NULL, risk_level TEXT NOT NULL, reason TEXT NOT NULL, status TEXT NOT NULL, decided_by TEXT, decided_by_name TEXT, decided_at TEXT, created_at TEXT NOT NULL);');
 }
...
export function createApproval(db,{runId,agentId,actionType,proposedOutput,riskLevel,reason,tenantId=null}) {
 if(!ACTION_TYPES.includes(actionType))fail(400,'نوع إجراء غير معروف للموافقة');
 const row={id:randomUUID(),tenantId:tenantId||resolveActiveTenantId(db),runId,agentId,actionType,proposedOutput,riskLevel,reason,status:'PENDING',createdAt:new Date().toISOString()};
 db.prepare('INSERT INTO agent_approvals (id,tenant_id,run_id,agent_id,action_type,proposed_output,risk_level,reason,status,created_at) VALUES (?,?,?,?,?,?,?,?,?,?)')
  .run(row.id,row.tenantId,row.runId,row.agentId,row.actionType,JSON.stringify(row.proposedOutput),row.riskLevel,row.reason,row.status,row.createdAt);
 return row;
}
