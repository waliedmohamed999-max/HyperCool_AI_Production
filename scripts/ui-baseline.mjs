...
try{
 ...
 await page.goto(base);await page.locator('#auth-form [name=name]').fill('مالك اختبار الواجهة');await page.locator('#auth-form [name=username]').fill('ui_owner');await page.locator('#auth-form [name=password]').fill('isolated-ui-test-password');await page.locator('#auth-form button').click();await page.locator('#protected').waitFor({state:'visible'});await page.waitForTimeout(1000);
