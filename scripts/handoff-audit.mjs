...
try{
 ...
 const base=`http://127.0.0.1:${app.server.address().port}`;await page.goto(base+'/app');for(const [name,value] of Object.entries({name:'حساب فحص التسليم',username:'handoff_owner',password:QA_PASSWORD}))await page.locator(`#auth-form [name=${name}]`).fill(value);await page.locator('#auth-form button').click();await page.locator('#overview-dashboard').waitFor();
 const auditContext=await browser.newContext({storageState:await context.storageState(),bypassCSP:true}),auditPage=await auditContext.newPage();await auditPage.goto(base+'/app');await auditPage.locator('#overview-dashboard').waitFor();
 for(const width of [1440,1920,768,390])for(const route of ['overview','crm','planning','reports','content','agents','knowledge','control-center','audit','users']){
